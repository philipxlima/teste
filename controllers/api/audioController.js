  ...
  try {
    return asyncAsync.retry(
      ...
      async function (err, result) {
        ...
        // Determine container and mimeType more reliably
        let container = "unknown";
        let mimeType = "application/octet-stream"; // Default generic type

        if (audio?.mimeType) {
            mimeType = audio.mimeType;
            if (mimeType.includes("webm")) container = "webm";
            else if (mimeType.includes("mp4")) container = "mp4";
            else if (mimeType.includes("mpeg")) container = "mp3";
            else if (mimeType.includes("ogg")) container = "ogg";
        } else if (audio?.container) {
            container = audio.container.toLowerCase();
            if (container.includes("webm")) mimeType = "audio/webm";
            else if (container.includes("mp4")) mimeType = "audio/mp4";
            else if (container === "mp3") mimeType = "audio/mpeg";
            else if (container === "ogg") mimeType = "audio/ogg";
        } else if (audio?.url) {
            const urlExtMatch = audio.url.match(/\.([^.?#]+)(\?.*)?$/);
            if (urlExtMatch) {
        ...
        if (mimeType === "application/octet-stream" && container === "unknown") {
             // Fallback if still unknown, but try to be more specific if possible
            if (audio?.url && audio.url.includes("googlevideo.com")) {
                mimeType = "audio/webm; codecs=\"opus\""; // Common for YouTube
                container = "webm";
            } else {
                mimeType = "audio/mpeg"; // A general fallback
                container = "mp3";
            }
        }
        ...
        // Remove potentially problematic headers from previous attempts
